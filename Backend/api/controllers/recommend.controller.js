  ...
  Preference.findById(userId, (preferenceErr, preferenceData) => {
    ...
    Userinfo.findById(userId, (myInfoErr, myInfoData) => {
      ...
      User.getAll((allUsersErr, allUsersData) => {
        ...
        const promises = allUsersData.map(
          (target) =>
            new Promise((resolve, reject) => {
              ...
              Userinfo.findById(targetId, (targetInfoErr, targetInfoData) => {
                ...
                Ownership.findByUserId(
                  targetId,
                  (ownershipErr, ownershipData) => {
                    if (ownershipErr) {
                      if (ownershipErr.kind !== "not_found") {
                        reject(ownershipErr);
                        return;
                      ...
                      Apt.findOne(ownershipData[0].AptID, (aptErr, aptData) => {
                        ...
                        checkUserPromise.then((matchScore) => {
                          if (
                            matchScore !== -1 &&
                            checkAptMatch(aptData, myInfoData)
                          ) {
                            recUsers.push({
                              user: target,
                              userInfo: targetInfoData,
                  ...
                  (ownershipErr, ownershipData) => {
                    if (ownershipErr) {
                      ...
                      checkUserPromise.then((matchScore) => {
                        if (
                          matchScore !== -1 &&
                          checkAptMatch(apt, targetInfoData)
                        ) {
                          recUsers.push({
                            user: target,
                            userInfo: targetInfoData,
...
const checkUserMatch = (preference, myInfo, targetInfo, resolve) => {
  if (
    preference.Gender !== targetInfo.Gender ||
  ...
  // using dummy code for development because there is a limit on calling monkeylearn api
  if (targetInfo.Comment.length >= 8) {
    resolve(2);
  } else {
    resolve(1);
  }
