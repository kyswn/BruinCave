  ...
  Ownership.findByUserId(req.params.UserID, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        ...
        res.status(500).send({
          message:
            "Error retrieving ownership with Userid " + req.params.UserID,
        });
      }
    } else res.send(data);
  });
...
exports.deleteByBothId = (req, res) => {
  Ownership.removeByBothId(req.params.UserID, req.params.AptID, (err, data) => {
    if (err) {
      ...
      res.send({
        message: `Ownership of Userid ${req.params.UserID}, AptId ${req.params.AptID} was deleted successfully!`,
      });
  });
};
  ...
  Ownership.removeByUserId(req.params.UserID, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        ...
        res.status(500).send({
          message:
            "Could not delete ownership with Userid " + req.params.UserID,
        });
      }
    } else
      res.send({
        message: `Ownership of UserId ${req.params.UserID} were deleted successfully!`,
      });
  });
};
...
exports.deleteByAptId = (req, res) => {
  Ownership.removeByAptId(req.params.AptID, (err, data) => {
    if (err) {
      ...
      res.send({
        message: `Ownership of AptId ${req.params.AptID} were deleted successfully!`,
      });
  });
};
